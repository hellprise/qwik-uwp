...
import type { QRL, NoSerialize, Signal } from "@builder.io/qwik";
...
export const useMediaRecorder = (options?: Options): HookReturn => {
  ...
  const store = useSignal<UnchangedStore>({
    mediaRecorder: null,
    mediaStream: null,
    audioContext: null,
    sourceNode: null,
  ...
  const startRecording = $(async () => {
    ...
    try {
      ...
      if (options?.transcipt?.enable) {
        store.value.recognizer = noSerialize(
        ...
        store.value.recognizer.continuous =
          options.transcipt.continuous ?? true;
        store.value.recognizer.interimResults =
          options.transcipt.interimResults ?? false;

        store.value.recognizer.onstart = (): void => {
          // setIsListening(true);
        };

        store.value.recognizer.onresult = (
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    if (statusRecording.value === "recording" && !timerId) {
      timerId = setInterval(() => duration.value++, 1000);
    } else {
